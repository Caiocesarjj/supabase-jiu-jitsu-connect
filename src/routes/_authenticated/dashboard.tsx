...
function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <WeightCategoryDistribution organizationId={organizationId} />
...
function WeightCategoryDistribution({ organizationId }: { organizationId: string | null }) {
  const { data, isLoading } = useQuery({
    queryKey: ["weight-cat-distribution", organizationId],
    queryFn: async () => {
      ...
      return { entries: Array.from(counts.entries()).sort((a, b) => b[1] - a[1]), missing };
    },
    enabled: !!organizationId,
  });
  ...
  return (
    <div className="rounded-xl border bg-card p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold">Alunos por categoria FBJJ</h2>
        {data.missing > 0 && (
          <span className="text-xs text-muted-foreground">
            {data.missing} aluno{data.missing === 1 ? "" : "s"} sem peso/sexo cadastrado
          </span>
        )}
      </div>
      {data.entries.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Cadastre peso e sexo nos alunos ativos para ver a distribuição.
        </p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {data.entries.map(([label, count]) => (
